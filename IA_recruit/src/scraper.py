"""
사람인(Saramin) 채용공고 데이터를 수집하여 SQLite 데이터베이스에 저장하는 스크립트입니다.
주요 기능:
- "내부감사" 검색 키워드 기반 1~10페이지 채용공고 목록 수집
- 각 채용공고의 상세페이지(view-detail API) 본문 텍스트 및 HTML 수집
- SQLite 데이터베이스에 UPSERT 방식으로 데이터 중복 제거 및 갱신 저장
- 수집 요약 및 결과를 마크다운 리포트로 자동 출력
"""
...
class SaraminScraper:
    def __init__(self, db_path="IA_recruit/data/saramin_recruit.db"):
        self.db_path = db_path
        self.headers = {
            "accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
            "accept-language": "ko-KR,ko;q=0.9,en-US;q=0.8,en;q=0.7,de-DE;q=0.6,de;q=0.5",
            "cache-control": "max-age=0",
            "referer": "https://www.saramin.co.kr/zf_user/search?search_area=main&search_done=y&search_optional_item=n&searchType=search&searchword=%EB%82%B4%EB%B6%80%EA%B0%90%EC%82%AC",
    ...
    def crawl_list(self, page=1):
        """
        특정 페이지의 사람인 채용공고 목록을 수집하여 DB에 저장합니다.
        """
        print(f"[목록 수집] {page}페이지 요청 중...")
        ...
        try:
            ...
            print(f"[목록 수집] {page}페이지에서 {len(items)}개의 공고 발견")
            ...
            for item in items:
                try:
                    ...
                    company_name = corp.get_text(strip=True) if corp else ""
                    
                    # 2. 공고 타이틀 및 URL
                    job_tit = item.select_one('.job_tit a')
                    if not job_tit:
                        continue
                    title = job_tit.get_text(strip=True)
                    href = job_tit.get('href', '')
                    url_full = "https://www.saramin.co.kr" + href if href.startswith('/') else href
                    
                    # rec_idx 파라미터 추출 (공고 ID)
                    match = re.search(r'rec_idx=([0-9]+)', href)
                    if not match:
                        continue
                    recruit_id = match.group(1)
                    
                    # 3. 채용 조건
                    conditions_el = item.select('.job_condition span')
                    conditions = ", ".join([c.get_text(strip=True) for c in conditions_el])
    ...
    def crawl_detail(self, recruit_id):
        """
        특정 공고 ID의 상세페이지(view-detail API)를 호출하여 DB에 적재합니다.
        """
        url = "https://www.saramin.co.kr/zf_user/jobs/relay/view-detail"
        params = {"rec_idx": recruit_id}
        
        headers = self.headers.copy()
        headers["referer"] = f"https://www.saramin.co.kr/zf_user/jobs/relay/view?rec_idx={recruit_id}"
        ...
        try:
            ...
            if user_content:
                ...
                # user_content 영역이 없을 시 바디 전체 텍스트 수집
    ...
    def crawl_all_details(self):
        """
        DB에 등록된 모든 공고 목록 중 상세 내용이 아직 없거나 갱신이 필요한 항목들을 순회하며 수집합니다.
        """
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
    ...
    def generate_report(self, report_path="IA_recruit/report/scraping_report.md"):
        """
        ...
        report_content = f"""# 사람인 채용공고 수집 결과 보고서
