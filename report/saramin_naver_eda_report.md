...
### 차트 11: 사람인 채용 공고 직무(job_group)별 핵심 키워드 TF-IDF 상위 30개 분석
![사람인 직무별 TF-IDF](../images/viz15_job_group_tfidf.png)

#### [기초 데이터 통계 테이블 (직무별 상위 10개)]
...
#### [차트 해석 및 분석]
사람인 채용 공고(수요)의 5대 직무(`job_group`)별 공고 제목과 세부 자격/역량 텍스트를 정제하여 TF-IDF 기반 텍스트 분석을 시도한 결과, 각 직무 고유의 기술 스택 및 소프트 스킬 요건이 확연하게 분류되는 것을 관찰할 수 있습니다.
- **인사·HR·총무**: 전통적인 인사(HR), 노무 관리뿐만 아니라 타 직무와의 '커뮤니케이션'과 '협업' 역량이 매우 중요한 스케일로 강조됩니다. 또한 기획 및 관리의 비중이 큽니다.
- **IT개발·데이터**: 소프트웨어 '개발'과 'AI', '데이터' 지수가 압도적으로 높으며, 구체적인 백엔드 스택인 `java`, `python`과 시스템 `설계` 및 `협업`이 핵심 키워드로 등장합니다.
- **회계·재무·경영관리**: '회계', '세무', '재무', '결산' 등 전문적인 수치 관리 업무에 절대적인 가중치가 쏠려있으며, 시스템 툴로 `erp` 및 `excel` 활용 능력이 기본적으로 우대됩니다.
- **마케팅·CRM**: 단순 광고 대행을 넘어 '마케팅 기획' 및 '브랜드 전략' 수립 역량이 강하게 부각되며, 특히 '데이터' 기반 고객 분석 역량이 필수 스펙으로 요구되는 추세입니다.
- **영업·사업개발**: '영업'과 '세일즈' 도메인 역량 외에 '기획', '전략', '계약' 검토와 고객/파트너사와의 원활한 '커뮤니케이션'이 핵심 경쟁력으로 작용합니다.

---

### 차트 12: 사람인 채용 공고 직무(job_group)별 핵심 키워드 워드클라우드 시각화
![사람인 직무별 워드클라우드](../images/viz16_job_group_wordcloud.png)

#### [차트 해석 및 분석]
앞서 산출된 직무별 TF-IDF 스코어를 가중치(frequencies)로 맵핑하여 워드클라우드로 시각화한 결과, 직무의 본질적 대표 키워드들이 직관적으로 파악됩니다. 
특히 모든 직무군을 관통하여 **'커뮤니케이션'**과 **'협업'**, **'데이터'** 키워드가 강하게 상주하고 있습니다. 이는 디지털 트랜스포메이션 시대를 맞아 직무를 막론하고 기본적인 데이터 문해력(Data Literacy)과 협업 중심의 소프트 스킬이 현대 화이트칼라 인재상에 필수 요소로 수렴하고 있음을 보여주는 정량적 증거입니다.
